...
namespace BookManger
{
    internal class BookManager
    {
        ...
        //借阅图书
        public Dictionary<string, dynamic> BorrowBook(string bookName)
        {
            // 借阅图书的逻辑处理
            // 定义一个图书字典
            Dictionary<string, dynamic> BorrowDic = new();
            // 判断文件是否存在====>不存在，返回"暂时没有书籍，请先添加"
            if (!File.Exists(path)) return BorrowDic;
            // 存在===> 读取文件====>反序列化===>根据名称查找====>找不到 ===> 返回"要借阅的书籍不存在，请先添加"
            var json = File.ReadAllText(path);
            List<Dictionary<string, dynamic>> list = JsonSerializer.Deserialize<List<Dictionary<string, dynamic>>>(json);
            Dictionary<string, dynamic> resDic = list.Find(item => item["name"].ToString() == bookName);
            if (resDic == null)
            {
                Console.WriteLine("要借阅的书籍不存在，请先添加");
                return BorrowDic;
            }  
            // 找到了===>判断当前图书的状态是否为可借阅
            if (resDic["isBorrow"].ToString() != "False")
            {
                return BorrowDic;
            }
            else{
                resDic["isBorrow"] = true;
            }
             // 将修改后的list序列化写回文件中
             File.WriteAllText(this.path, JsonSerializer.Serialize(list, this.JsonOpts));
             return resDic;
        ...
        //归还图书
        public Dictionary<string, dynamic> ReturnBook(string bookName)
        {
            // 归还图书的逻辑处理
            // 定义一个图书字典
            Dictionary<string, dynamic> ReturnDic = new();
            // 判断文件是否存在====>不存在，返回"暂时没有书籍，请先添加"
            if (!File.Exists(path)) return ReturnDic;
            // 存在===> 读取文件====>反序列化===>根据名称查找====>找不到 ===> 返回"要归还的书籍不存在，请先添加"
            var json = File.ReadAllText(path);
            List<Dictionary<string, dynamic>> list = JsonSerializer.Deserialize<List<Dictionary<string, dynamic>>>(json);
            Dictionary<string, dynamic> resDic = list.Find(item => item["name"].ToString() == bookName);
            if (resDic == null)
            {
                Console.WriteLine("要归还的书籍不存在，请先添加");
                return ReturnDic;
            }
            // 找到了===>判断当前图书的状态是否为可归还
            if (resDic["isBorrow"].ToString() != "True")
            {
                return ReturnDic;
            }
            else
            {
                resDic["isBorrow"] = false;
            }
             // 将修改后的list序列化写回文件中
             File.WriteAllText(this.path, JsonSerializer.Serialize(list, this.JsonOpts));
             return resDic;
